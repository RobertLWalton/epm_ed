...
So what point on the finite line {\tt pq} is closest to the circle?
If there is an intersection, that is such a point and the distance is zero.
...
circle is a circle, so that later we can use these arguments for
an ellipse.  Instead we will use the fact that the circle, viewed
as an area, is convex.

\begin{definition}
A set $S$ of the points in a plane (i.e., $S\subset{\mathcal R}^2$)
is \key{convex} if and only if for any two points $p$ and $q$ in $S$,
the finite
line segment $pq$ is in $S$ (i.e., for any scalar $0\le\alpha\le 1$,
$\alpha*p+(1-\alpha)*q \in S$).

The \key{boundary} of a set $S$ of the points on a plane is the
set of points $p$ such that every sufficiently small circle with
center $p$ contains a point in $S$ and another point not in $S$.

A set $S$ of the points on a plane is \key{closed} if every point
...
\begin{lemma}
Let $S$ be a close set and $p$ a point not in $S$.  Then there
is some point $q$ in $S$ which is closest to $p$; that is,
a $||q-p||\ge||r-p||$ for every $r$ in $S$.
\end{lemma}

We will \underline{not} give the proof of this,
which involves the mathematics of compact
sets (i.e., uses the fact that a circle is compact).
...
function, i.e., given two points $r_1$ and $r_2$ on $pq$ outside $S$ and
$0\le\alpha\le 1$,
$D(\alpha*r_1+(1-\alpha)*r_2)\le\alpha*D(r_1) + (1-\alpha)*D(r_2)$.
\end{lemma}
\begin{indpar}{0.2in}
Proof: \\
~\begin{tabular}[t]{l}
...
\begin{lemma}
Let $D(r)$ be a convex function on points $r$ on an infinite line
which is bounded from below: that is, there is a scalar $b$ such
such that $D(r)\ge b$ for every $r$.  Then $D(r)$ has a minimum:
that is, there is an $r_0$ such that $D(r)\ge D(r_0)$ for every $r$.
\end{lemma}

This is another lemma we will \underline{not} prove, as it
involves using mathematics to prove convex functions are
continuous.

\begin{lemma}
Let $D(r)$ be a convex function on points $r$ on an infinite line
and let $r_0$ be a point where $D$ is a minimum.  Then $D$ is
(non-strictly) monotone increasing on either side of $r_0$ (i.e., as one moves
away from $r_0$).
\end{lemma}
\begin{indpar}{0.2in}
Proof by Contradiction:
Suppose $r_1$ is to one side of $r_0$, $r_2$ if further to that
side, and $D(r_1)>D(r_2)$.  Then for some $0\le\alpha\le 1$,
$r_1=\alpha*r_0+(1-\alpha)*r_2$, so since $D$ is convex,
$D(r_1)\le\alpha*D(r_0)+(1-\alpha)*D(r_2)
\le \alpha*D(r_2)+(1-\alpha)*D(r_2)=D(r_2)$, contradiction.
\end{indpar}

Given the above we have:

\begin{theorem}
The distance from a finite line $pq$ to a convex set $S$ when
$pq$ is outside of $S$ is one of:
\begin{enumerate}
...
\begin{indpar}{0.2in}
Proof: If (1) applies this is obvious.  Otherwise by the
previous lemma $D$ is monotone on $pq$ and so must take
its minimum at $p$ or $q$.
\end{indpar}


Now we have enough information to find the distance between
a finite line {\tt pq} and a circle when {\tt pq} is \underline{outside} the
circle.
...
The convex hull is obviously the union of all finite lines {\tt pq} for
all pairs of points {\tt p} and {\tt q} in $S$.  The convex hull of
a convex set $S$ is obviously $S$ itself.
If $T$ is a subset of a convex set $S$,
the convex hull of $T$ is obviously a subset of $S$.
...
Date:         & Sun Nov 22 01:48:05 EST 2020
