...
To make further progress, we are not going to use the fact that the
...
\begin{definition}
A subset $S$ of the points in a plane (i.e., $S\subset{\mathcal R}^2$)
...
\begin{lemma}
Let $S$ be a convex set and $pq$ be an infinite line.  For $r$ on $pq$,
define $D(r)$ to be the distance from $r$ to $S$ (distance of
$r$ to the point in $S$ closest to $r$).  Then $D(r)$ is a convex
...
So now we have enough information to find the distance between
a finite line {\tt pq} and a circle when {\tt pq} is outside the
circle.
...
Date:         & Sat Nov 14 00:45:46 EST 2020
