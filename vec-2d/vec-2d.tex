...
at point $r_i$.  Then $r_1=r_2=$ the intersection of the infinite
...
a finite line {\tt mn} intersects an infinite line {\tt pq}
if and only if:
\\[1ex]
...
The case where {\tt mn} and {\tt pq} are parallel, that is, when: \\
\centerline{{\tt (q-p)\#(n-m)==0:(2d)}} \\
requires a different approach.  If {\tt n} is not on the infinite
line that extends {\tt pq}, that is if: \\
\centerline{{\tt 0!=(q-p)\#(n-q):(2d)}} \\
then there is no intersection.  Otherwise both {\tt nm} and {\tt pq}
are on the same infinite line and may or may not overlap.
We can use the {\tt (q-p)*} product to establish coordinates on this
infinite line, getting: \\
\centerline{\tt (q-p)*m ~~~ (q-p)*n ~~~ (q-p)*q ~~~ (q-p)*r} \\
Then by exchanging {\tt m} with {\tt n} and {\tt q} with {\tt r}
\underline{if necessary} we can arrange that: \\
\centerline{\tt (q-p)*m<(q-p)*n:(2d) ~ and ~ (q-p)*q<(q-p)*r:(2d)} \\
and there will be an overlap if and only if it is \underline{not}
true that \\
\centerline{\tt (q-p)*n<(q-p)*q:(2d) ~ or ~ (q-p)*r<(q-p)*m:(2d)} \\
If there is an overlap, the overlap interval will be: \\
\centerline{\tt [ max\{(q-p)*m,(q-p)*q\}, min\{(q-p)*n,(q-p)*r\} ]} \\
expressed in {\tt (q-p)*} coordinates.  Since the points on our
line have the form {\tt q + s*(q-p)}, a point on the line with
{\tt (q-p)*} coordinate {\tt S} would be such that \\
\centerline{\tt (q-p)*q + s*(q-p)*(q-p) = S} \\
...
Date:         & Thu Dec 31 03:28:20 EST 2020
