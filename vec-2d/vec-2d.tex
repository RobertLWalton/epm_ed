...
\begin{indpar}{0.2in}
Proof: By Lemma~\ref{MINIMUM-DISTANCE} for each $r$ in $pq$ the
distance $D(r)$ from $r$ to $S$ is well defined and $D(r)$ has
a minimum, i.e., there is an $r_0$ in $pq$ such that $D(r)\ge D(r_0)$
for every $r$ in $pq$.

TBD
Proof: If (1) applies this is obvious.  Otherwise
there is a point 
line $pq$ 
by the
