...
Date:         & Sat Jul 24 12:00:41 EDT 2021
\end{tabular}
\\[2ex]
\begin{tabular}{p{5in}}
...
\begin{lemma}
If $b\in I$ then {\tt M'($b$).y = 0} implies $D=0$ at $b$;
and also {\tt N'($b$).y = 0} implies $D=0$ at $b$.
\end{lemma}

\begin{indpar}{0.3in}
Proof of Lemma:

If {\tt M'($b$).y = 0}, either {\tt M' = M} and {\tt M} is on
the line {\tt PQ}, or {\tt M'.x = P.x} or {\tt M'.x = Q.x} and
{\tt P} or {\tt Q} is on the line {\tt MN}, so in either case
$D=0$.  Similarly if {\tt N'($b$).y = 0} then $D=0$.
\end{indpar}
...
(c) & {\tt M'($b_t$).y > 0 > N'($b_t$).y} \\
(d) & {\tt M'($b_t$).y < 0 < N'($b_t$).y} \\
\end{tabular}
\end{center}
Note that {\tt M'($b_t$).y > 0 < N'($b_t$).y}
is impossible as $b\in T$,
and similarly {\tt M'($b_t$).y < 0 > N'($b_t$).y} is impossible.

So we need only prove that (c) and (d) are impossible.  There are
exactly two cases:
...
(g) & if $b_s\in s$ then {\tt M($b_s$).x < P.x > N($b_s$).x}; \\
    & therefore by continuity
            {\tt M($b$).x $leq$ P.x $geq$ N($b$).y}; \\
    & therefore since $b\in T\subset I$,
            {\tt M'($b$).x = P.x = N'($b$).x}; \\
(h) & if $b_s\in s$ then {\tt M($b_s$).x > Q.x < N($b_s$).y} \\
    & therefore by continuity
