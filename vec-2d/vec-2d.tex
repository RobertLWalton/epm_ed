...
\tt azm v & returns $(180/\pi)*\mathrm{atan2}(v_y,v_x)$ & [azimuth] \\
...
defined by infinite lines passing through a common point?
\end{enumerate}
...
to two other directions, {\tt pq$_1$}  and {\tt pq$_2$} (see picture).
The infinite lines {\tt pq$_1$}  and {\tt pq$_2$} divide space into
four regions:
\begin{itemize}
\item[LR]
...
Thus, for example,
if {\tt p} is in region LR, then when {\tt pq$_1$} is rotated
counter-clock\-wise
...
\tt onf~pqrd & returns {\tt true} if and only if {\tt r} is on the
                \underline{finite} line {\tt pq}, \\
	      & if all coordinates have at most {\tt d} decimal places \\
\tt between~pquvd & returns {\tt true} if and only if the directed
                   line {\tt pu} \\
		 & rotated counter-clockwise will line up with
		   the \\
		 & directed line {\tt pq} before it lines up with {\tt pv}; \\
		 & return false if {\tt pq} has the same
		   direction as {\tt pu} or {\tt pv}; \\
	      & if all coordinates have at most {\tt d} decimal places \\
\end{tabular}
\end{center}
...
Date:         & Sun Jun 20 12:52:41 EDT 2021
