...
It preserves angles and multiplies distances by {\tt ||q-p||}.
...
\end{center}
and we define {\tt X} and {\tt Y} to be the coordinates of {\tt R}
so {\tt R = (X,Y)}.
...
may be as large as \\
\centerline{\tt 10$^{\tt d}$*(numerator of (q-p)*(q-p))}
...
\begin{scope}[>=triangle 45,shorten >=0.01in]
    ...
    \draw[black] (+3.0,-2.0) node{\small \tt L = (p-q)*(p-q)};
...
If {\tt X<0}, the answer
is {\small \tt ||R-P||/||q-p||=||r-p||}.  If {\tt X>||q-p||},
the answer is {\small \tt ||R-Q||/||q-p||=||r-q||}.
...
Date:         & Wed Dec 30 02:43:36 EST 2020
