...
{\em variables} (beginning with a two-character {\em operator}).
\end{itemize}
...
or this rotation followed by the dilation {\tt ||q-p||}.  We use
...
\end{center}
where we have defined {\tt X} and {\tt Y} to be the coordiates of {\tt R}
...
is {\tt ||R-P||}.  If {\tt X>||q-p||}, the answer is {\tt ||R-Q||}.
...
\hspace*{0.3in}{\tt Y==0:(2d)},~~~{\tt 0<=X:(2d)},~~~and
               {\tt X<=(p-q)*(p-q):(2d)}
\end{enumerate}
...
Date:         & Sat Oct 31 06:05:59 EDT 2020
\end{tabular}
