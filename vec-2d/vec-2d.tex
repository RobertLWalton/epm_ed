...
\begin{scope}[>=triangle 45,shorten >=0.01in]
    ...
    \fill[blue] (5.0,0) circle(0.1);
    \draw[blue,->] (0,0) -- (+5.0,0);
    \draw[black] (5.5,0) node{$v$};

    \fill[red] (0,0) circle(0.1);
    \draw[black] (2.0,-0.5) node{$||w||*\cos \theta$};
    ...
    \fill[red] (3.464101615,2.0) circle(0.1);
    \draw[red,->] (0,0) -- (3.464101615,2.0);
    \draw[black] (1.3,+1.5) node{$||w||$};
    \draw[black] (4.0,+2.0) node{$w$};

    \draw[orange] (3.464101615,0) -- (3.464101615,2.0);
    \draw[black] (3.464101615,1) + (0.1,0)
                 node[right]{$||w||*\sin \theta$};
\end{scope}
\end{tikzpicture}
\end{wrapfigure}
We can use this lemma to give a geometric characterization
of our two products.  Let $v$ and $w$ be two vectors and let
$\theta=\mathrm{asm}~w - \mathrm{asm}~v$ be the direction angle of $w$
relative to $v$.  If we rotate both vectors by $R^{-\mathrm{asm}~v}$
we line $v$ up with the positive X-axis, while not changing $\theta$,
$||v||$, $||w||$, $v*w$, or $v\#w$.  So we have
(see the picture): \\
\hspace*{0.5in}
    \begin{tabular}{l}
    $v=(||v||,0)$ \\
...
\end{tikzpicture}
\end{wrapfigure}
We see that $v*w$ is just $||v||$ times the projection $||w||\cos\theta$ of
$||w||$ onto the direction of $v$.  If $v$ is a unit vector, $u$ with
$||u||=1$, this is just the projection of $||w||$ onto the direction of $u$.
TBD
\end{minipage}
...
Date:         & Thu Oct 22 02:09:24 EDT 2020
\end{tabular}
