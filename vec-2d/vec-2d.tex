...
\tt X:v & returns $v_x$ ({\tt X} is \underline{not} a variable here) \\
\tt Y:v & returns $v_y$ ({\tt Y} is \underline{not} a variable here) \\
...
\\[1ex]
\tt \#!point p [color] [label] & point {\tt p} \\
\tt \#!line pq [color] [label] & line from point {\tt p}
                                 to point {\tt q} \\
\tt \#!arrow pq [color] [label] & arrow from point {\tt p} to point {\tt q} \\
\tt \#!infline pq [color] [label] & infinite line through points \\
                          & {\tt p} and {\tt q} \\
\tt \#!infarrow pq [color] [label] & infinite directed line \\
                           & through points {\tt p} and {\tt q} \\
			   & with arrow from {\tt p} to {\tt q} \\
\tt \#!circle cr [color] [label] & circle of radius {\tt r} and center {\tt c} \\
\tt \#!line L  [color] & lines with endpoints that are \\
...
values must be vectors denoting points; {\tt r} is an arbitrary
{\em variable} whose value must be a strictly positive scalar;
{\tt L} is a {\em variable} whose value must be a list of points.
A `{\tt label}' is text used to label a point, line, arrow, or
center of a circle.
...
\tt \#!newpage [label] & Starts new logical page \\
                       & with label in page header \\
\tt \#!header label & Adds label to current page header \\
\end{tabular}
\end{center}
...
(e.g., {\tt 00-000-vec-2d}).  Each `{\tt label}' adds one
...
Date:         & Tue Dec 15 03:52:42 EST 2020
