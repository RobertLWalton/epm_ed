...
Date:         & Sat Jul 24 04:47:26 EDT 2021
\end{tabular}
\\[2ex]
\begin{tabular}{p{5in}}
...
there is a point $d\in B_b$ with $D=0$.
\end{lemma}
...
as functions on points of $L$.  These functions are continuous on $L$,
though {\tt M'} and {\tt N'} may not exist for some points of $L$.
We will use proof by contradiction, and therefore assume that $D\neq 0$
for all points of $L$.  Then:

\begin{center}
\begin{tabular}{l}
if {\tt M'} exists at $b\in L$, then {\tt M'($b$) $\neq$ 0} \\
if {\tt N'} exists at $b\in L$, then {\tt N'($b$) $\neq$ 0} \\
{\tt M'($b_t$)} and {\tt N'($b_t$)} exist \\
either {\tt M'($b_t$).y > 0 > N'($b_t$).y}
    or {\tt M'($b_t$).y < 0 < N'($b_t$).y} \\
if {\tt MN} intersects $S$ at $b_f\in L$, then either \\
~~~{\tt M'($b_f$).y > 0 < N'($b_f$).y}
   or {\tt M'($b_f$).y < 0 > N'($b_f$).y} \\
\end{tabular}
\end{center}

Case 1: {\tt MN} intersects $S$ for all points on $L$.

Then {\tt M'} and {\tt N'} exist for all $b\in L$ and
...
Which ever of these cases is true, by continuity of
{\tt M'} and {\tt N'} we have that either {\tt M'($b$).y = 0}
or {\tt N'($b$).y = 0} for some $b\in L$, a contradiction.
...
has a boundary point $b_j$ between $b_t$ and $b_f$ such that
{\TT MN} intersects $S$ for all $b\in L$ that are between
$b_t$ and $b_j$.  Let $L_j$ denote the line between $b_j$ and $b_t$,
and note that since {\tt MN} intersects $S$ for $b\in L_j$, then
{\tt M'($b$)} and {\tt N'($b$)} exist for all $b\in L_j$.

As $b_j$
(which is not to be confused with $b_b$)
is a boundary point of $I\cap L$,
one of the following is true at $b_j$:
\begin{center}
\begin{tabular}{l}
{\tt M.x = Q.x $\leq$ N.x} \\
{\tt N.x = Q.x $\leq$ M.x} \\
...
We will prove that $D=0$ for some $b\in L_j$.  We give the proof
for just the first of these four cases,
as the proof for each of the other three cases is similar.
	
\begin{indpar}{0.3in}
Case 2a: {\tt M.x = Q.x < N.x} at $b_j\in L_j$

Then {\tt N' = M} at $b_j$ so the following are true:
\begin{center}
\begin{tabular}{l}
{\tt M'($b_j$).y = N'($b_j$)}.y \\
\begin{tabular}{@{}rl}
...
{\tt M'} and {\tt N'} on $L_j$,
either {\tt M'($b$).y = 0}
or {\tt N'($b$).y = 0} for some $b\in L_j\subset L$.


Case 2b: {\tt M.x = Q.x = N.x} at $b_j\in L_j$

Since $b_j\in L_j\subset T$,
\begin{center}
\begin{tabular}{l}
\begin{tabular}{@{}rl}
either & {\tt M'($b_j$).y > 0 > N'($b_j$).y} \\
    or & {\tt M'($b_j$).y < 0 < N'($b_j$).y} \\
\end{tabular} \\
{\tt Q.y = 0} \\
\end{tabular}
\end{center}
so {\tt Q} is on {\tt MN} at $b_j$ and therefore $D=0$ for $b_j\in L_j$.
\end{indpar}
