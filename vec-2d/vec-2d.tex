...
\\[1ex]
\tt \#!point p [color] & point {\tt p} \\
\tt \#!line pq [color] & line from point {\tt p}
                                 to point {\tt q} \\
\tt \#!arrow pq [color] & arrow from point {\tt p} to point {\tt q} \\
\tt \#!infline pq [color] & infinite line through points \\
                          & {\tt p} and {\tt q} \\
\tt \#!infarrow pq [color] & infinite directed line \\
                           & through points {\tt p} and {\tt q} \\
			   & with arrow from {\tt p} to {\tt q} \\
\tt \#!circle cr [color] & circle of center {\tt c}
                                   and radius {\tt r} \\
				 & (this must be strictly \\
				 & positive) \\
\tt \#!ellipse cRA [color] & ellipse with center {\tt c}, radii \\
                                   & {\tt R.x} and {\tt R.y} (these \\
				   & must be strictly positive), \\
                                   & rotated by angle A about its \\
				   & center \\
\tt \#!points L  [color] & points in the list of points {\tt L} \\
\tt \#!lines L  [color] & lines with endpoints that are \\
                        & consecutive points in the \\
		        & list of points {\tt L} \\
\tt \#!polygon L  [color] & the polygon whose vertices are \\
                       & the points in the list of points {\tt L} \\
\tt \#!text p [color] D text... & the text, positioned at point {\tt p} with \\
                       & displacement as per {\tt D} (see below) \\
\end{tabular}
\end{center}
...
For text {\tt D} is one or two letters displacing text from the point
{\tt p = ($x$,$y$)} as follows:
\begin{indpar}{0.5in}\begin{tabular}{l}
`{\tt l}' or `{\tt r}' to place text horizontally
    to the left or right of $x$; \\
~~~ if neither, text is horizontally centered on $x$ \\
`{\tt t}' or `{\tt b}' to place text vertically
    on top of or on the bottom of $y$; \\
~~~ if neither, text is vertically centered on $y$ \\
`{\tt x}' if none of the above letters are given,
    to center text on {\tt p} \\
...
Date:         & Wed Dec 16 23:45:06 EST 2020
