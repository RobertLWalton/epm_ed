...
{\tt p + s*(q-p)} and using \\
\centerline{\tt (n-m)\#(q-p) - (n-m)\#(m-p) = (n-m)\#(q-m)}
...
As {\tt MN} intersects $S$ for $b_t$, and {\tt MN} is a continuous
function in $L$, then for some point $b_i$ on $L$ we must have:
\begin{indpar}{0.3in}
Case 2a: {\tt M.x = Q.x} and {\tt N.x > Q.x}.  Consequently
{\tt N' = M} so as in Case 1 one of {\tt M'.y} and {\tt N'.y}
must be {\tt 0} for some point in the interval $[b_t,b_i]$.
\\
Case 2b: {\tt M.x = Q.x = N.x}.  Then {\tt Q} is on {\tt MN} so
$D=0$ for $b_i$.
\\
Other Cases: Like Case 2a or 2b but with {\tt P} and {\tt Q} and
{\tt <} and {\tt >} switched, or with
{\tt M} and {\tt N} switched.

\end{indpar}
The fact that $b_i$ exists and falls into one of these cases
involves inductively defining an open covering of the compact
set $L$ and observing that all open coverings of $L$ have a
finite subcovering: this is mathematics we will not go into.
\end{indpar}
...
Date:         & Tue Jul  6 11:52:18 EDT 2021
